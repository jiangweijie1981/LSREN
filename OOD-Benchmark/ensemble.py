...
def ensemble(args,model_list):
    ...
    dataset_list=['Imagenet','Imagenet_resize', 'LSUN', 'LSUN_resize', 'iSUN', 'SVHN','Gaussian', 'Uniform',\
        '{}_out'.format(args.in_dataset),'{}_noise'.format(args.in_dataset)]
    if args.in_dataset=='cifar10':
        dataset_list.append('cifar100')
    elif args.in_dataset=='cifar100':
        dataset_list.append('cifar10')
...
def get_acc(args,model_list):
    ...
    accs=[]
    for i in range (len(model_list)):
        each_score = torch.load('{}/score/{}_out_Pred.pt'.format(model_list[i],args.in_dataset))
    ...
    acc=100-100.*incorrect/nTotal
    with open('{}/acc_{}.txt'.format(args.save,args.infer),'wt') as f:
        print('acc:{:.2f}'.format(acc),file=f)
        print('{},accs:'.format(args.save),file=f)
        print(accs,file=f)


def vote_acc(args,model_list):
    for i in range (len(model_list)):
        each_score = torch.load('{}/score/{}_out_Pred.pt'.format(model_list[i],args.in_dataset))
        each_onehot=torch.nn.functional.one_hot(torch.max(each_score,dim=1)[1],args.class_num)
        if i==0:
            total_score = each_onehot
        else:
            total_score += each_onehot
    Pred=torch.max(total_score,dim=1)[1]

    in_dataloader=get_dataloader(args,'in')
    acc=get_acc(args,in_dataloader)
    total_target=torch.empty(0,dtype=int).cuda()
    ...
    incorrect = Pred.ne(total_target.data).cpu().sum()
    nTotal = len(in_dataloader.dataset)
    print(100-100.*incorrect/nTotal)
    with open('{}/info.txt'.format(args.save),'wt') as f:
        print('soft_acc:{:.2f}, voc_acc:{:.2f}'.format(acc,100-100.*incorrect/nTotal),file=f)
...
if __name__=='__main__':
    ...
    if args.train_type=='ILSR':
        ensemble_path = '../models/{}_seed{}_{}_{}{}/{}_esbl{}_range{}_penalty{}/ensemble_epochs{}'.format(
            args.net,args.seed,args.in_dataset,args.noise_type,args.noise_ratio,
